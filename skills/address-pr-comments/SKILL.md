...
description: >-
  Work through the unresolved review comments on a pull request: find the PR,
  read what's still open, fix the straightforward ones and push to the PR's own
  branch, and escalate the judgment calls. Use when asked to "address the PR
  comments", "handle the review feedback", "resolve the comments on my PR",
  "respond to the reviewer", or similar. Closes the loop on review feedback —
  every thread ends with a reply naming the commit that fixed it.
---
...
cite; the explanation is the close.
